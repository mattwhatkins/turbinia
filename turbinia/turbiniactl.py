...
def main():
  ...
  # PSQ Worker
  parser_worker = subparsers.add_parser('psqworker', help='Run PSQ worker')
  ...
  # Celery Worker
  parser_worker = subparsers.add_parser('celeryworker', help='Run Celery worker')
  ...
  if evidence_ and args.server:
    ...
    if args.dump_json:
      ...
      if args.use_celery:
        log.info(
            'Creating Kombu request {0:s} with evidence {1:s}'.format(
                request.request_id, evidence_.name))
        task_manager_.kombu.send_request(request)
      else:
        log.info(
